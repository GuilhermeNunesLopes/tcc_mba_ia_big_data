...
def main():
    ...
    for n_components in SVD_COMPONENTS:
        ...
        # ==========================================
        # INJEÇÃO DA FEATURE DE VOLUME (LOG BURST)
        # ==========================================
        scaler_vol = MinMaxScaler()
        vol_train = scaler_vol.fit_transform(df_train[['Qtd_Logs_Na_Janela']])
        vol_test = scaler_vol.transform(df_test[['Qtd_Logs_Na_Janela']])
        
        X_train_misto = np.hstack((X_train_svd, vol_train))
        X_test_misto = np.hstack((X_test_svd, vol_test))

        # A TRANSFORMAÇÃO FINAL: Padronização Z-Score do espaço latente
        scaler_final = StandardScaler()
        X_train_final = scaler_final.fit_transform(X_train_misto)
        X_test_final = scaler_final.transform(X_test_misto)
        ...
        resultado_treino = anomaly_detector.process_log_anomalies(
            ...
            X_tfidf=X_train_final,   # <--- Usando a matriz concatenada
        ...
        resultado_teste = anomaly_detector.process_log_anomalies(
            df_original=df_test,
            X_tfidf=X_test_final,    # <--- Usando a matriz concatenada
        ...
        if metricas["F1_Score"] > best_f1:
            ...
            best_threshold_final = threshold_treinado
            X_test_final_ablation = X_test_final # Guarda para o bloco de ablation mais abaixo
