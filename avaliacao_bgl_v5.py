...
def main():
    ...
    print("\n[2/4] Engenharia de Atributos: TF-IDF Esparso + Metadados (Sem SVD)...")

    # TÉCNICA 1: Injeção de Metadados (Tamanho físico do log)
    df_train['log_length'] = df_train['Raw_Log'].apply(len)
    df_test['log_length'] = df_test['Raw_Log'].apply(len)

    # TÉCNICA 2: Injeção de Metadados (Tamanho físico do log)
    df_train['log_length'] = df_train['Raw_Log'].apply(len)
    df_test['log_length'] = df_test['Raw_Log'].apply(len)

    # TÉCNICA 2: O "Hack" da Severidade
    ...
    # Escalonamento dos Metadados para [0, 1]
    scaler_meta = MinMaxScaler()
    meta_train = scaler_meta.fit_transform(df_train[['log_length', 'is_error']])
    meta_test = scaler_meta.transform(df_test[['log_length', 'is_error']])

    # FUSÃO: TF-IDF Esparso Direto (sem perder palavras raras) + Metadados
