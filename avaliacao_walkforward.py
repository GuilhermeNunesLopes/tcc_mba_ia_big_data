...
def coletar_logs(pastas_ativas, resumos_parse_saida=None):
    """
    Varre as pastas configuradas e retorna o DataFrame concatenado de logs
    parseados pelo Drain3.
    ...
    for pasta in pastas_ativas:
        if os.path.exists(pasta):
            ...
            for path in read_generic:
                ...
                # (main_v4/v5/v6), que usa o mesmo nome de pasta como
                # nome_fonte e mantém um estado do Drain3 persistido e
                # "aquecido" com o histórico inteiro do arquivo. Se a
                # avaliação walk-forward reusasse esse mesmo estado
                # (drain3_state_<pasta>.bin), os splits mais antigos se
                # beneficiariam de templates que só existem porque o motor ao
                # vivo já tinha processado o arquivo inteiro antes — inclusive
                # trechos que, para aquele split, representam "o futuro". Um
                # nome de fonte próprio grava num arquivo .bin separado,
                # nunca tocado pelo motor ao vivo.
                nome_da_fonte = os.path.basename(pasta) + "_walkforward_eval"

                # Além de isolar do motor ao vivo, apaga o estado de uma
                # execução anterior desta MESMA avaliação antes de começar:
                # sem isso, uma segunda rodada do walk-forward herdaria os
                # templates aprendidos na primeira (que já tinha visto o
                # arquivo até o fim) — mesmo tipo de vazamento, só que entre
                # execuções do próprio script. Cada execução aprende os
                # templates do zero, só a partir da leitura sequencial do
                # arquivo (que já respeita a ordem cronológica) — ou seja, o
                # template usado numa linha nunca depende de linhas futuras
                # dentro desta mesma execução.
                ...
                gerador_lotes = parse_system.automatic_drain_parse(
                    path, nome_fonte=nome_da_fonte, tamanho_lote=100000,
...
def rodar_avaliacao_walkforward(pastas, algoritmo="iforest", reducao="pca",
    ...
    resumos_parse = []
    df_logs = coletar_logs(pastas, resumos_parse_saida=resumos_parse)
    if df_logs.empty:
