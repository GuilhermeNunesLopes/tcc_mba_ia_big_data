...
def main():
    ...
    print("\n[2/4] Engenharia de Atributos: Amplificação de Metadados...")
    ...
    # Isso força o algoritmo aleatório a olhar para o comportamento físico e temporal.
    meta_train_amplified = np.hstack([meta_train])
    meta_test_amplified = np.hstack([meta_test])

    # FUSÃO: TF-IDF Esparso + Metadados Amplificados
    X_train_final = sp.hstack((X_train_tfidf, meta_train_amplified), format='csr')
    X_test_final = sp.hstack((X_test_tfidf, meta_test_amplified), format='csr')


    # 1. PROCESSAR TREINO 
    # ==========================================
    print("\nFase de Treino: Otimizando Motor...")
    resultado_treino = anomaly_detector.process_log_anomalies(
        df_original=df_train,
        X_tfidf=X_train_final,
        y_true=df_train["y_true"], 
        model=None,
        algorithm="ocsvm" # <--- MUDE AQUI PARA TESTAR O OCSVM OU "iforest" PARA VOLTAR
    )
    ...
    plt.plot(recalls, precisions, label=f"One-Class SVM (PR-AUC = {pr_auc:.3f})")
    ...
    caminho_figura = os.path.join("resultados", "curva_pr_bgl_svm.png")
