...
def main():
    ...
    print("\n[2/4] Injetando Metadados Físicos (Tamanho do Log)...")
    # Cria uma nova feature com o tamanho (em caracteres) da string processada
    df_train['log_length'] = df_train['Raw_Log'].apply(len)
    df_test['log_length'] = df_test['Raw_Log'].apply(len)

    # Escalonamos o tamanho para ficar entre 0 e 1 (mesma escala do TF-IDF)
    scaler_len = MinMaxScaler()
    len_train = scaler_len.fit_transform(df_train[['log_length']])
    len_test = scaler_len.transform(df_test[['log_length']])

    # Concatena a matriz esparsa do TF-IDF com a nova coluna de tamanho
    X_train_final = sp.hstack((X_train_tfidf, len_train), format='csr')
    X_test_final = sp.hstack((X_test_tfidf, len_test), format='csr')
