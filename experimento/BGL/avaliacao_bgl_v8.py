...
def carregar_bgl_rotulado(caminho: str = CAMINHO_BGL, limite_linhas: int = None, resumo_saida=None) -> pd.DataFrame:
    ...
    with open(caminho, "r", encoding="utf-8", errors="ignore") as f:
        if limite_linhas:
            # Mantém a ordem cronológica original — só corta a cauda do arquivo,
            # em vez de amostrar aleatoriamente (o TimeSeriesSplit exige ordem).
            linhas = list(itertools.islice(f, limite_linhas))
        else:
            linhas = f.readlines()
    ...
    # Sem isso, contagens de ocorrência por template ficam infladas/erradas
    # ao longo de múltiplas execuções (estado do Drain3 nunca é resetado).
    nome_fonte_isolado = "BGL_Eval_v8_isolado"
    ...
    # 2. Converte o gerador em um DataFrame do Pandas.
    # automatic_drain_parse é um gerador que faz yield de UM DataFrame por
    # lote (ver parse_system.py) — passar o gerador direto para pd.DataFrame()
    # empilha os lotes como um array 3D (lotes x linhas x colunas) e quebra
    # com "Must pass 2-d input" sempre que há 1 ou mais lotes (bug pré-existente,
    # nunca chegava a rodar até o fim). Precisamos concatenar os lotes.
    df_parsed = pd.concat(list(gerador_parsed), ignore_index=True)
    os.remove(temp_path)
...
def main(limite_linhas=None, dataset="200k"):
    print("=" * 62)
    ...
    for n_comp in SVD_COMPONENTS:
        ...
        for train_index, test_index in tscv.split(df_bruto):
            ...
            # iForest — treina no fold inteiro, sem limite (escala bem).
    ...
    pasta_saida = run_output.criar_pasta_execucao("bgl_v8_tscv_iforest_vs_svm")
    caminho_json = os.path.join(pasta_saida, "resultados_bgl_v8.json")
    with open(caminho_json, "w", encoding="utf-8") as f:
        json.dump(resultados_finais, f, indent=4, ensure_ascii=False)
    print(f"\n✅ Resultado salvo em: {caminho_json}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Avaliação BGL (TimeSeriesSplit + iForest vs OCSVM).")
