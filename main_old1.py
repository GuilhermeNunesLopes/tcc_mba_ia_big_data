...
if __name__ == "__main__":
    ...
    for i in read_generic:
       ...
       print(df_generic_parse.head()) # Verificando o conteúdo do DataFrame parseado
    ...
    print(f"Dimensões Apache: {X_apache.shape} | Dimensões Spark: {X_spark.shape} | Dimensões Genéricos: {X_generic.shape}")
    ...
    dataframe_result["no_anomaly_generic"] = df_generic_final.loc[normal_generic.index, ['Level', 'Event']]
    dataframe_result["no_anomaly_apache"] = df_apache.loc[normal.index, ['Level', 'Event']]
    print("\n" + "="*30)
...
#
  ...
  #  print(f"Anomalias Logs Genéricos: {len(anomalies_generic)}")
    ...
    normal_apache_final = normal.sum(axis=1)
    normal_spark_final = normal_spark.sum(axis=1)  
    normal_generic_final = normal_generic.sum(axis=1)
    
    dataframe_result["anomaly_spark"] = df_spark.loc[normal_spark.index, ['Level', 'Event']]
    dataframe_result["anomaly_generic"] = df_generic_final.loc[normal_generic.index, ['Level', 'Event']]
    dataframe_result["anomaly_apache"] = df_apache.loc[normal.index, ['Level', 'Event']]
    ...
    df_final = pd.DataFrame({
        ...
        "Normal_Apache": normal_apache_final.reset_index(drop=True),
        "Normal_Spark": normal_spark_final.reset_index(drop=True),
        "Normal_Generic": normal_generic_final.reset_index(drop=True)
    })
