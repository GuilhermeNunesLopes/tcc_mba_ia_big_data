...
def processar_logs_em_lote():
    ...
    # Recebe as variáveis dinâmicas do portal (incluindo a técnica de redução)
    ...
    # 2. Redução de Dimensionalidade Dinâmica (PCA vs SVD)
    if reducao_ativa.lower() == "svd":
        print(f"[{time.strftime('%H:%M:%S')}] 📉 Aplicando TruncatedSVD...")
        red_train, red_model = preprocessor.apply_truncated_svd(tfidf_train, svd_model=None, n_components=100)
    else:
        print(f"[{time.strftime('%H:%M:%S')}] 📉 Aplicando PCA...")
        red_train, red_model = preprocessor.apply_pca(tfidf_train, pca_model=None, n_components=100)
    
    # 3. Normalização das Features Temporais com Transformação Logarítmica (Treino)
    scaler = StandardScaler()
    temp_train = scaler.fit_transform(np.log1p(df_train[['time_delta', 'log_rate_5m']]))

    # 4. Combinação das matrizes
    ...
    # 2. Aplica a mesma redução de dimensionalidade (SVD ou PCA)
    if reducao_ativa.lower() == "svd":
    ...
    # 3. Aplica a mesma escala temporal com Transformação Logarítmica (Teste)
...
if __name__ == "__main__":
    ...
    print("🚀 Iniciando o Sistema de Detecção de Anomalias (v3)...")
