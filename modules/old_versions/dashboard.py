...
def main():
    ...
    if os.path.exists("config.json"):
        ...
        if tempo_json > tempo_dados:
            ...
            if st.session_state.espera_motor < 30: # Aumentei as tentativas para 30
                st.session_state.espera_motor += 1
                st.warning(f"⏳ **IA analisando novos dados...** (Tentativa {st.session_state.espera_motor}/30)")
                with st.spinner("O Motor está gerando o modelo. Aguarde..."):
                    # OTIMIZAÇÃO: Checa a cada 10 segundos em vez de 30!
                    time.sleep(10) 
                    st.rerun()
            else:
