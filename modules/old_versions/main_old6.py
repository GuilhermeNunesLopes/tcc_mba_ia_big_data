...
def processar_logs_em_lote():
    ...
    # Ordenação cronológica final obrigatória
    ...
    # ==========================================
    # NOVA LÓGICA DE TREINO E TESTE (Sem Vazamento)
    # ==========================================
    ...
    df_train, df_test, X_train, X_test = train_test_split(
        df_logs, matriz_combinada, test_size=0.2, shuffle=False # <--- shuffle=False GARANTE O SPLIT CRONOLÓGICO
    )
    ...
    if qtd_anomalias > 2:
        ...
        X_anomalias_denso = X_test[mask_anomalias].toarray()
